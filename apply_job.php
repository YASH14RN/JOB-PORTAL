...
<!-- CSS End -->
 ...
 if(isset($_POST['submit'])){
    ...
    move_uploaded_file($_FILES["file"]["tmp_name"],'admin/files/'.$file);

    $sql=mysqli_query($conn,"SELECT * FROM job_apply WHERE job_seeker='$job_seeker' AND id_job='$id_job'");
    if(mysqli_num_rows($sql)>0){
      echo "<h3>You have already Applied !!</h3>";
    }
else{
    $query=mysqli_query($conn,"INSERT INTO job_apply(first_name,last_name,dob,files,id_job,job_seeker,mobile_number)
    VALUES('$first_name','$last_name','$dob','$file','$id_job','$job_seeker','$mobile_number')");
    if($query){ ?>
     <p class="lead">Your Application has been Submitted. Thanks for Applying.</p>

    <?php 
    }else{
    echo "<script> alert('Please try again $file')</script>";
    }
    }
  }
?>
