...
<?php
 ...
 <main role="main" class="col-md-9 ml-sm-auto col-lg-10 pt-3 px-4">
  ...
  <ol class="breadcrumb">
    ...
    <form action="phpmailer.php" method="post" style=" border:1px solid gray; width:80% ; margin-left:5%  ;padding:10px; ">
        <?php
        include('connection/db.php');
        $id=$_GET['id'];
      ...
      $query=mysqli_query($conn,"SELECT * FROM job_apply LEFT JOIN all_jobs ON job_apply.id_job = all_jobs.job_id WHERE id='$id'");
       ...
       while($row=mysqli_fetch_array($query)){
        ...
        <div class="form-group">
          ...
          <td><textarea name="body" id="body" cols="125" rows="10">   Dear <?php echo strtoupper($row['first_name']) ;?> <?php echo strtoupper($row['last_name']) ;?>,
          </textarea></td>
        </div>
               
            <?php }  ?>
            <input type="submit" class="btn btn-success" name="submit" id="submit" value="Send" ></input>
